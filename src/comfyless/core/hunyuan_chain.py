...
minimizing artifacts" (Tencent README §Architecture). ADR-025's
...
- ADR-025 §3 (2026-05-24 amendment): original framing retraction
...
def load_refiner_pipeline(
    ...
    *,
    ...
    Family-aware VAE tiling resolution per ADR-025's tile-VAE-skip
...
def build_refiner_call_kwargs(
    ...
    *,
    ...
    `height`/`width` MUST be forwarded: HunyuanImageRefinerPipeline.__call__
    defaults a missing dimension to `default_sample_size * vae_scale_factor`
    (64 × 16 = 1024 for the refiner's 16× VAE) and then RESIZES the input
    image to that size via `image_processor.preprocess(image, height,
    width)`. Omitting them silently collapsed every refined image to
    1024×1024 regardless of the requested resolution. Pass the base
    output's actual dimensions so the refiner preserves the resolution the
    base produced.
    """
    kwargs: Dict[str, Any] = {
        "prompt": prompt,
...
def run_chain(
    ...
    *,
    ...
    # Refine at the resolution the base actually produced. base_pil.size is
    # the source of truth (== the base call's aligned height/width); passing
    # it prevents the refiner from defaulting to 1024×1024 and downscaling
    # the base output. PIL .size is (width, height).
    refiner_kwargs = build_refiner_call_kwargs(
        refiner_pipe, base_pil, prompt, negative_prompt,
        refiner_steps, refiner_cfg, generator,
        height=base_pil.height, width=base_pil.width,
    )
