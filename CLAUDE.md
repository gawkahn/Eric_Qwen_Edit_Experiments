...
python3 test_params_schema.py               # 142 tests: comfyless COMFYLESS_SCHEMA + adapters
python3 test_cascade.py                     # 129 tests: comfyless Stable Cascade dispatch (ADR-010)
python3 test_machine_boundary_validator.py  # 148 tests: machine-boundary validator (ADR-012)
...
python3 test_mcp_server.py                  # 158 tests: comfyless MCP server (ADR-011 slice 1)
python3 test_hunyuan.py                     # 244 tests: Hunyuan-Image 2.1 family + tile-VAE-skip + refiner chain (ADR-014/016)
```
All ten suites run against the comfyless uv-managed `.venv` — invoke via `./.venv/bin/python3` (created by `uv sync` at the repo root; see ADR-013 for the dep-divergence rule). Total 1289 unit tests; expect 0 failures.
