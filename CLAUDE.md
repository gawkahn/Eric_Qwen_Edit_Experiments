...
python3 test_server_robustness.py           #  48 tests: comfyless IPC timeouts + BrokenPipe survival + device-keyed socket routing (ADR-020)
...
```
All ten suites run against the comfyless uv-managed `.venv` — invoke via `./.venv/bin/python3` (created by `uv sync` at the repo root; see ADR-013 for the dep-divergence rule). Total 1568 unit tests; expect 0 failures.
