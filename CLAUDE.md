...
python3 test_quant.py                       #  85 tests: fp8 quantize-on-load (ADR-019 slice A) — eligibility policy, cache-key discrimination, LoRA merge guard, boundary hygiene
```
All ten suites run against the comfyless uv-managed `.venv` — invoke via `./.venv/bin/python3` (created by `uv sync` at the repo root; see ADR-013 for the dep-divergence rule). Total 1497 unit tests; expect 0 failures.
