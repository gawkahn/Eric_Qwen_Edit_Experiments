...
python3 test_refine.py                       #  94 tests: refinement-loop verdict boundary + catalog layer (ADR-027 slices 1-2) — closed two-key override allowlist (F1: path-bearing keys + lora path dropped), numeric hygiene (F6: NaN/Infinity/huge-int rejected, weights |w|≤4, scores 1-10), reject-unknown + critique allowlist + verdict coercion (F7), judge image downscale + seed-image byte/pixel caps (F5); LoRA name→path ONLY via ADR-015 resolver (F2: unknown/wrong-kind dropped), path-stripped planner metadata + structural AST guard against load-plane column reads (F3), open_catalog_db degrades on corrupt DB
```
All eighteen suites run against the comfyless uv-managed `.venv` — invoke via `./.venv/bin/python3` (created by `uv sync` at the repo root; see ADR-013 for the dep-divergence rule). Total 2718 unit tests; expect 0 failures.
