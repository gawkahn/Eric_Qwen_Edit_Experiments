...
python3 test_params_schema.py               # 136 tests: comfyless COMFYLESS_SCHEMA + adapters
python3 test_cascade.py                     # 129 tests: comfyless Stable Cascade dispatch (ADR-010)
python3 test_machine_boundary_validator.py  # 128 tests: machine-boundary validator (ADR-012)
...
python3 test_hunyuan.py                     # 109 tests: Hunyuan-Image 2.1 family + tile-VAE-skip (ADR-014)
```
All ten suites run against the comfyless uv-managed `.venv` — invoke via `./.venv/bin/python3` (created by `uv sync` at the repo root; see ADR-013 for the dep-divergence rule). Total 1128 unit tests; expect 0 failures.
