...
Rules:
- **`pyproject.toml` and `requirements.txt` must agree on direct deps at all times** — both list the same 10 top-level pins in the same order (`torch`, `diffusers`, `transformers`, `accelerate`, `peft`, `safetensors`, `pillow`, `numpy`, `mcp`, `click`). Any dep bump edits both.
