...
def _build_call_kwargs(
    ...
    if model_family == "hunyuan-image":
        ...
        # (see ADR-014 §2). negative_prompt is accepted by the pipeline; its
        # docstring says "Ignored when not using guidance" — forward it when
        # set and let the pipeline decide (ADR-014 §5). max_sequence_length
...
class EricDiffusionGenerate:
    ...
    def generate(
        ...
        #   - refiner_path unset + hunyuan-image   → loud stdout warning,
        #                                            base-only run
        ...
        if refiner_path:
            ...
            # ("loud stderr warning" — same stream as the comfyless side).
            # ComfyUI-flavored: references the node's refiner_path input
            # rather than the CLI --refiner flag. Two of the three Vision
            # Intent fragments are shared with the comfyless warning
            # ("hunyuan-image quality requires a refiner" + the
            # huggingface-cli download line); the third diverges
            # ("set refiner_path on the Generate node" vs. CLI's
            # "pass --refiner <path>"). Locked at runtime by the Step 3
            # ComfyUI-parity structural assertions in test_hunyuan.py.
            import sys as _sys
            print(
                "[EricDiffusion] WARNING: hunyuan-image quality requires a "
                "refiner; set refiner_path on the Generate node; download "
                "with huggingface-cli download "
                "hunyuanvideo-community/HunyuanImage-2.1-Refiner-Diffusers",
                file=_sys.stderr,
            )
        ...
        # When the Hunyuan-Image refiner chain is active, run_chain
        # handles both pipeline calls under a single swap_sampler context.
        # The swap is per-pipe (operates on base only); the refiner's
        # scheduler is untouched (ADR-016 §(g) / Vision Inv 8).
        try:
        ...
        if refiner_pipe is not None:
            # Two-stage metadata extension per ADR-016 §(h). Keys are
            # absent (not present-and-empty) on base-only runs —
            # Vision Inv 4.
