...
# N19 iterate second-site: RETIRED 2026-07-10 (ADR-012 amendment). The
# `--iterate lora` file was reclassified from a machine boundary to a
# human-authored replay surface; its normalizer (_normalize_iterate_lora_element)
# legitimately does its own isinstance dispatch on str/dict/list, so the
# isinstance-free invariant no longer applies to it. The server-side N19 above
# still guards the actual wire. Iterate leniency is proven in test_iterate.py.

# ──────────────────────────────────────────────────────────────────────
print("\n== N18 iterate parity: RETIRED — iterate is a human surface, not the wire ==")

# The 11 canonical↔iterate parity fixtures were removed 2026-07-10 (ADR-012
# amendment). Iterate no longer routes through validate_lora_entry: it is a
# hand-authored replay file that defaults a missing weight to 1.0 and accepts
# "path:weight" strings, DELIBERATELY diverging from the strict wire validator.
# We assert the divergence here so the decoupling can't silently regress, and
# confirm the canonical validator itself stays strict (unchanged by the amendment).
import comfyless.generate as gen_mod  # noqa: E402

check(
    "N18-retired: wire validator still REJECTS a missing weight (unchanged)",
    validate_lora_entry({"path": "/x.sft"}, 0).ok is False,
)
check(
    "N18-retired: human iterate ACCEPTS a missing weight, defaults 1.0 (diverges)",
    gen_mod._normalize_iterate_lora_element({"path": "/x.sft"}, 0)
    == [{"path": "/x.sft", "weight": 1.0}],
)
