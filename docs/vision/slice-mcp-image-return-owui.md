...
3. **Returned base64 is size-bounded (pixel).** When `return_image=true`, the returned image's longest edge is ≤ `max_return_px` (default **768**); a 50 MP Qwen generation does not yield a 50 MP base64. The **full-resolution PNG on disk is NOT downscaled** — the bound applies only to the transport copy.
3b. **Returned base64 is size-bounded (bytes).** `len(image_b64)` ≤ the effective `max_return_bytes` = `min(requested, 1 MiB)`. A request may ask for a smaller cap; a larger value is clamped to the 1 MiB ceiling (true hard cap, not agent-raisable). When the pixel-bounded copy still exceeds the byte budget, `_encode_return_image` iteratively downscales the **transport copy only** until it fits. The on-disk PNG is never shrunk to meet the byte budget. (Honors the local_agents scope-A "tool bounds its own output" convention; keeps the payload under the local_agents MCP proxy's 1 MiB result cap / 4 MiB frame cap should comfyless ever be consumed there — see ADR-017 Changelog.)
...
- `return_image=true` → `image_b64` present, base64-decodes to a valid PNG, longest edge ≤ `max_return_px` (default 768).
...
- A transport copy whose base64 exceeds the byte cap (force a tiny `max_return_bytes`, e.g. 4096) → returned `len(image_b64)` ≤ the effective cap, the image still decodes to a valid (smaller) PNG, **and the on-disk PNG is still full-res** (assert all three).
- `max_return_bytes` larger than the 1 MiB ceiling → clamped: a huge requested value does not raise the effective cap above 1 MiB (assert the effective behavior, e.g. payload still ≤ 1 MiB).
- The base64 payload string does **not** appear in the captured stderr/audit line.
- `return_image` non-bool / `max_return_px` non-int / `max_return_bytes` non-int → `ValidationError` before generation.
...
| **1** | **comfyless base64 return.** Add `return_image` (bool, default false) + `max_return_px` (int, default 768) + `max_return_bytes` (int, server ceiling 1 MiB, clamped) to the generate schema; a `_encode_return_image(output_path, max_px, max_bytes) -> (b64, mime)` helper (Pillow pixel-downscale + iterative byte-downscale + PNG encode); wire into both handlers' response building; tests (all negative cases incl. byte-cap + clamp). | code-reviewer + security-auditor (Opus) |
...
- **OQ-1.** ~~`max_return_px` default — proposing **1024**~~ **RESOLVED 2026-06-25 (post-local_agents-spec audit):** default **768** + a hard **`max_return_bytes`** ceiling of 1 MiB on the base64 payload (clamped, agent may only lower). The lower pixel default makes the iterative byte-downscale almost never fire; the byte cap matches the local_agents scope-A "tool bounds its own output" convention and the MCP-proxy 1 MiB result cap. See ADR-017 Changelog.
