...
11. **Enrichment is by resolved name, read-only, and never in the resolution path.** After the references resolve to live-catalog names, and only when a metadata DB is available (step 4a auto-discovery or explicit `--catalog-db`), the handler looks each resolved name up in the DB (read-only) and surfaces the results as a **separate top-level `enrichment` object keyed by catalog name** — `{name: {classification?, model_family?, description?}}`, `description` = `{source, model_name, text, usage_tips, strength_rec, sampler_rec, trigger_words}` (present fields only). It is a *separate* map, not inline in `params`, so `params` stays purely replay-ready (a `model`/`transformer` name is a bare string; a `loras[]` entry stays `{name, weight}`). The `enrichment` key is present only when at least one name enriched. The DB is queried by name only — it never resolves a path and never influences which name was chosen. Only catalog **hits** are enriched: a basename-fallback (miss) reference and an HF-repo passthrough are **not** (so enrichment cannot probe DB contents for an arbitrary agent-supplied basename).
12. **Enrichment strictly allowlists fields, never leaks a path, and never fails the call.** Each `enrichment[name]` contains only the fields above; it never carries `abs_path`, `root`, `relative_path`, or any filesystem string (the reverse-lookup reuses `catalog_db.search`'s `SELECT e.*` row but projects an explicit allowlist, never `dict(row)`). `trigger_words` is re-validated on read (list of ≤64 short strings) exactly as `_handle_search` does. DB unavailable, name-absent, or DB-read error → no `enrichment` entry for the affected name (invariant 8's no-leak guarantee holds regardless), and a top-level `try/except` around the enrichment call makes "enrichment degrades, never fails the extract" a **structural** guarantee (the `params` result always stands; both-reviewer fold).
...
- **Enrichment degrades, never fails:** any DB-read problem (no DB, name absent, corrupt row, connect error) — and, via the handler's top-level guard, any unexpected exception — yields a response with no `enrichment` entry for the affected name (or no `enrichment` key at all) — never an error, never a leaked DB path. The `params` result always stands.
...
20. **`scaffolding_repo`, `config_source`, `output_path`, and all runtime keys are dropped.** `scaffolding_repo` is the operator-default architecture config, not agent-affectable (mirrors the generate-cascade removed-field rule + `_resolved_cascade_params_as_names`); `config_source`/`output_path` are abs paths; `iterate_batch_id`/`run_id`/`run_index`/`total_runs`/`timestamp`/`elapsed_seconds`/`prior_seconds`/`decoder_seconds` are runtime noise. None appear anywhere in the response (no key, no value). *(`run_id` added 2026-07-27, ADR-040 D1b — it is the run-correlation primitive now stamped into every sidecar. Recorded here as the second of D1b's two mandatory registration duties; note the guarantee is STRUCTURAL rather than list-driven — the non-cascade branch normalizes through `_validate_params`, which drops every non-`COMFYLESS_SCHEMA` key, and the cascade branch renders through the positive allowlist in item 19. This entry documents an existing property and the tripwire in `test_mcp_server.py` pins it.)*
