...
8. **No absolute path or directory crosses the boundary, in any field or notice.** Every path-typed field is either rewritten to a name/basename or dropped. Specifically the handler:
   ...
   - rewrites `loras[]` to `[{name, weight, …enrichment}]`, dropping the `path` key;
   ...
   - for a cascade sidecar, rewrites `cascade_config.stage_*` → names and drops `scaffolding_repo` (mirroring `_resolved_cascade_params_as_names`).
