...
2. **Exact restore on unload — LIFO order** *(amended per security review DMR-5)* — quantized backups store the ORIGINAL representation verbatim (torchao: the original `Parameter` object; ScaledFp8Linear: `(w_fp8, weight_scale)` clones — fp8-sized, cheaper than today's bf16 clones) and restore by swap, never by re-derivation. Unload returns the model to bit-identical pre-merge weights. **Direct-merge unload is LIFO-only** — this was always true of the plain-path backups (non-LIFO unload destroys the newer adapter's delta) and becomes structurally binding under Parameter swaps; `unload_adapters` WARNS loudly on non-LIFO order (warn-don't-block) via a merge-order ledger on the transformer. **Assumption (DMR-6): merge targets have no weight tying and no accelerate offload hooks holding Parameter references; a post-swap aliasing assert enforces the reachable-object invariant, and introducing either feature requires revisiting the swap semantics.** (Negative tests: merge → unload → bit-equal originals, both reps; non-LIFO unload warns.)
...
- `nodes/eric_qwen_edit_lora.py` — the three `_load_*_adapter_direct` merge loops route through the dispatcher; `model_sd` becomes `{**named_parameters(), **named_buffers(.weight-only)}` *(merge order and the `.weight`-only filter are BINDING — security review DMR-3/DMR-8: unfiltered buffers would let adversarial LoRA keys like `foo.weight_scale.diff` resolve onto scale buffers and poison them via the plain path)*; `unload_adapters` restore branch dispatches kind-tagged backups + LIFO warning.
