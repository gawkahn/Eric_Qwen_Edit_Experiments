...
3. **Scale integrity** — every fp8 weight that has a scale in the file keeps that exact scale through key-remap; a quantized layer whose scale went missing is a LOUD load error, never a silent scale=1.0. (Negative test: header with weight but stripped scale → error naming the layer.)
...
7. **No trust-class change at the machine boundary** — MCP/agent callers still supply catalog NAMES only; the new loader runs on operator-curated files under the scanned roots (ADR-015/018 containment unchanged). Nothing in this slice adds a new caller-controllable path input.
...
`security-auditor` on THIS DESIGN before code (caller-supplied weight-file parsing — malformed headers, scale-key aliasing across remap, adversarial safetensors are in scope for the review). Then `code-reviewer` on the diff before commit, per slice-A pattern.
