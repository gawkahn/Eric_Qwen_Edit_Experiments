...
**Date:** 2026-06-25
**Status:** accepted (design settled with Grant 2026-06-25; spike-validated. Implementation gated by `code-reviewer` + `security-auditor`, both Opus, per slice — see Changelog.) Steps 1–3 implemented + e2e-validated 2026-06-27.
...
- **2026-06-27 (steps 2–3 implemented + e2e-validated):** The OpenWebUI native tool landed at `comfyless/integrations/openwebui/generate_image_tool.py` (+ install README) and is validated end-to-end — a generated image renders inline in OpenWebUI (proven with gpt-oss-120b driving the tool). It calls mcpo `/generate` with `return_image=true`, uploads the bytes to OWUI's file store under the caller's identity (`upload_file_handler`, `process=False`), and emits `![](/api/v1/files/{id}/content)`; the model receives only a short confirmation. Three runtime gaps found only in live e2e were fixed (each `code-reviewer` + `security-auditor` gated): async `Users.get_user_by_id` must be awaited; `upload_file_handler` FastAPI sentinel params (`metadata`, `process_in_background`) must be passed explicitly; the bounded response read must iterate `iter_chunked` (a single `read(n)` truncated the multi-chunk frame). The tool also gained read-only catalog tools — `list_models` / `list_loras` / `list_transformers` (catalog names + family, never paths) — so the model can discover models before generating. mcpo is launched via the new repo-root `start-mcpo.sh` (model-base = `hf-local`, the curated 39-name set). Reviews: `docs/security/review-adr017-owui-tool-2026-06-26.md` (incl. Follow-up B). Deferred: "list LoRAs/transformers in the same family as model X" — scan-derived LoRAs/transformers carry no family (manifest-only); deferred pending the separate LoRA-catalog project (see TECH_DEBT 2026-06-27). The OOM/VRAM + LoRA-apply fix on the MCP generate path is a separate change (`docs/security/review-mcp-pipeline-cache-2026-06-27.md`).
