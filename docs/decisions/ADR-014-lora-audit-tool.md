...
- **2026-07-28 (live-run fix — best-matching base wins, not alphabetically-first)**: The first real audit over the 831-LoRA corpus exposed a defect that every unit test had missed, and that the reviewed diff shipped with. Found because Grant refused to push before a live verification run.

  **Defect.** `_resolve_bases` hands bases out **alphabetically** (`sorted(bases.items())`, `scripts/lora_audit.py:453`) — not in config order, which the amendment's code comment wrongly asserted. The probe loop returned on the FIRST base that matched. Architecturally related families match the same file (Chroma is a Flux.1 derivative and shares `FluxLoraLoaderMixin`), so `chroma` beat `flux` on every genuine Flux LoRA purely by sort order. Measured on `kdny_body_flux.safetensors`: **`flux` 100% key match, `chroma` 84.62%, reported `chroma`.** Corpus-wide, **58 of 138** converted files carried the wrong family. That tag is exactly what feeds catalog search, MCP `list_loras`, and the refine planner's offers — the surfaces ADR-041 will be tuned against.

  **Fix.** Probe every base; select with `_native_hit_rank` (`key_match_pct` → `dim_ok_pct` → OK-over-NORM_TARGETING). Coverage deliberately outranks the verdict label: the motivating file is `flux` 100% NORM_TARGETING versus `chroma` 84.62% OK, and ranking the label first re-selects the wrong base. Ties stay deterministic (invariant 8) because `max` runs over the alphabetically-ordered base list. `native_convert.matched_bases` now records every base that matched, restoring the multi-family signal the direct path keeps in `verdicts_by_base` and that first-hit was silently discarding.

  **Result of the re-audit** (identical totals; only attribution moved): `flux` 0 → **58**, `chroma` 66 → **8**, `flux2` 10, `flux2klein` 25, `krea` 37. **66 files match both `chroma` and `flux`** — the real family-conflict population, now visible instead of collapsed.

  **Independent verification.** Grant's heuristic — a LoRA's directory usually names its parent model, exceptions are rare and deserve extra scrutiny — was used as a check, never as classifier input (classifying by path would be unsound). Of 138 converted entries: 114 agree with their directory, 23 sit in directories the heuristic does not map, 1 disagrees. All 23 unmapped and the 1 disagreement were then checked against each file's own training metadata (`modelspec.architecture` / `ss_base_model_version`), which the classifier never reads — **138/138 confirmed correct**. The single disagreement, `Flux.2 D/concept/Petite_body_type.safetensors`, is a **filing error, not a misclassification**: its metadata reads `modelspec.architecture: flux-1-dev/lora`, `ss_base_model_version: flux1`, `ss_network_module: networks.lora_flux`, trained 2024-12-21 at hidden dim 3072. The classifier was right and the folder was wrong.

  **Lesson worth keeping:** the unit suite could not have caught this. Every fixture used a single base, so first-hit and best-match were indistinguishable; the bug only exists when two related families are configured together, which is the production configuration. `test_native_convert_picks_best_base_not_first` now pins it with two bases where the alphabetically-first one is deliberately worse. A second self-correction on my part: an early spot-check of "all 138 match their directory" was made by sampling the first six per base and generalising, and was wrong — the full sweep showed 58 mismatches. Sampling is not verification.

  Cost: the probe no longer short-circuits, so a fall-through file now runs up to 9 converters instead of stopping at the first hit. Accepted — the state dict is already resident and the audit is not on any latency path.

  Proof: `test_lora_audit.py` 237 checks, `just tests` 29/29, pyright roots unchanged. Mutation-tested — reverting `max(...)` to `native_hits[0]` turns the new test red.

  **`code-reviewer` fold-in (Fable — the cost prohibition lifted 2026-07-28 evening).** Returned not approved; three findings, all folded:

  - **R-1 (MED-HIGH) — the family fix never reaches the catalog.** The load-bearing catch. `comfyless/catalog_builder.py:175-182` derives a lora's `ok_bases` *solely* from `verdicts_by_base` filtered to OK/NORM_TARGETING, and reads top-level `matched_bases` only for `kind: "transformer"`. It never touches `native_convert` — confirmed by grep: the key appears nowhere under `comfyless/`. A native-convert entry's direct verdicts are all WRONG_ARCH by construction, so `ok_bases` is empty and the catalog falls back to the **path/sidecar hint** — the very heuristic this ADR calls unsound as classifier input. Consequence: the 58-file correction, and the 66-file family-conflict population, are real in the manifest and invisible to catalog search, MCP `list_loras`, and refine offers. `Petite_body_type.safetensors` would be tagged `flux2` by its wrong folder with the classifier's correct `flux` answer discarded. Folded in two parts: the misleading code comment now states the signal is *carried for* — not read by — the catalog, and the consumer wiring lands as its own commit (below) rather than widening this diff. Reviewer's assessment that recording only usable hits is semantically correct (it parallels `ok_bases`, which also discards WRONG_ARCH) is accepted — the semantics were right, the wiring was absent.

  - **R-2 (MED) — `key_match_pct` compares different denominators across mixins.** Percentage is computed over the post-conversion dict, whose size each mixin decides. A converter that drops unrecognised keys can retain half the source (passing `_COVERAGE_FLOOR`), match all of that sliver at 100%, and outrank the correct family's fuller conversion at 90%. Fixed rather than documented, because it is the same bug class as the defect this entry exists to correct: `_native_hit_rank` now ranks on **`matched` (absolute resolved layers) first**, which is denominator-free. The opposite bias is named in the docstring, not hidden — qkv-splitting inflates absolute counts — and judged weaker, since split layers must still resolve against the base index to count. Within a single mixin the metrics are monotonically equivalent, so the flux-vs-chroma case is unaffected. **Re-audited to confirm: 0 base assignments and 0 classifications changed across the full 831-file corpus** — pure hardening, no live behaviour change.

  - **R-3 (MED-LOW) — the "coverage outranks the verdict label" decision had no pin.** `test_native_convert_picks_best_base_not_first` happened to grade *both* its bases NORM_TARGETING, so the rank tuple's label element was never exercised: reordering to label-first — the plausible "fix" a future reader of `_is_usable_verdict` might make — left the whole suite green while re-selecting the wrong family in production. Added `test_native_convert_coverage_outranks_verdict_label`, a miniature of the live case (clean OK at partial coverage vs NORM_TARGETING at 100%), with explicit premise assertions so it fails loudly if diffusers' grading shifts rather than passing vacuously. Mutation-checked: label-first reordering turns it red.

  Nits folded: the `FileEntry.native_convert` docstring now lists all five keys. Not folded, deliberately: `_try_native_convert_match`'s reason element is vestigial at its only call site (behaviour-identical, and removing it churns the signature for nothing), and the intra-`native_convert` `matched_bases` name collides with the transformer-entry field of the same name — nested placement disambiguates, and renaming would break the schema additivity just established. Per-base repeated conversion for mixin-sharing families (`flux`+`chroma`, `flux2`+`flux2klein`) is real but accepted with the cost note above.

  Final proof: `test_lora_audit.py` **241 checks**, `just tests` 29/29, pyright roots unchanged at `comfyless=13 nodes=520 pipelines=454`.

- **2026-07-28 (R-1 consumer wiring — native-convert families now reach the catalog)**: Separate commit, separate edit scope, because it touches `comfyless/catalog_builder.py` rather than the audit tool. `ok_bases` for `kind: "lora"` is now the **union** of the direct `verdicts_by_base` OK/NORM_TARGETING set and `native_convert.matched_bases`. Without it the ADR-014 amendment was inert past the manifest: a native-convert entry's direct verdicts are all `WRONG_ARCH`, so `ok_bases` came out empty and family resolution fell through to the sidecar/path hint.

  Pinned by a `test_catalog_db.py` fixture that reproduces the production shape exactly — a lora with `WRONG_ARCH` against every base, family evidence only in `native_convert.matched_bases`, and a **deliberately disagreeing sidecar** so the test cannot pass via the fallback it is meant to prevent. Mutation-checked: dropping the union term yields `model_family: "flux2"` (the wrong sidecar answer) with `family_conflict: None` — silently wrong and unflagged, which is what the corpus would have had. With the union: `qwen-image`, conflict recorded. `just tests` 29/29; pyright roots unchanged.

  **Follow-on, same live run: the ranked winner was still being discarded.** With the union wired, the rebuild put `Petite_body_type.safetensors` in `chroma` — not the `flux` the audit had ranked. Cause: `ok_bases` is an unordered set, so the multi-base tiebreak (sidecar → path → `ok_families[0]`) never saw the audit's ranking, and this file's sidecar *and* path both name `flux2` (the misfiled folder), so both heuristic rungs missed and alphabetical-first chose `chroma`. `native_convert.base` is now carried through the join as `native_base` and inserted as a rung **above alphabetical, below sidecar/path** — the 2026-07-06 sidecar>path precedence is deliberate and tested, so only the arbitrary last resort was replaced with a measurement. Worth recording that the first attempt at the pinning fixture **passed vacuously** — its ranked winner also happened to sort alphabetically first, so the mutation check (not the green suite) caught it; the ranked base is now deliberately the alphabetically-later one.

  **Final live state.** Searchable loras **241 → 310**. Catalog family distribution for native-convert entries now matches the manifest exactly: `flux` 58, `krea` 37, `flux2klein` 25, `flux2` 10, `chroma` 8. `flux` overall went 10 → 68 — it was the most under-served family in the catalog, which is precisely what a Kohya-format blind spot predicts. The acid test reads `model_family: "flux"`, `family_conflict: "chose flux; disagreeing: path=flux2"` — correct family, folder disagreement flagged rather than silently followed.
