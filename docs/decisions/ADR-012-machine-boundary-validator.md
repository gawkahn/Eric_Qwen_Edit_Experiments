...
- **2026-05-16 (implemented)**: Validator slice landed in 5 commits (`58ef335` step 1 → `57bd650` step 4). All three consumers named in §1 wired to the canonical validator:
  - **`comfyless/params_validation.py`** (NEW, step 1) — `validate_machine_request` + `validate_lora_entry`, frozen `ValidationResult` dataclass, `MappingProxyType`-wrapped canonical schema maps. 72 dedicated tests covering Vision N1-N17, N20-N21, N25-N30 + parametric bool-reject coverage. `security-auditor`: CLEAN with 2 MEDIUMs folded in (`MappingProxyType` immutability + parametric coverage extension).
  - **`comfyless/params_schema.py`** (NEW, step 2) — thin adapter that builds `COMFYLESS_SCHEMA` from `SCHEMA_KIND` + `_FIELD_DEFAULTS`. Schema collapse landed: `cfg_scale` / `true_cfg_scale` / LoRA `weight` all canonical-`float`. Dependency direction cycle-free: `params_validation` (no project deps) → `params_schema` → `generate`. CLI `_validate_params` invariant 7 preservation verified (acceptance set unchanged; warn set grew on `cfg_scale=int` and `cfg_scale=bool`).
  - **`comfyless/server.py:_validate_request`** (step 3) — thin wrapper around `validate_machine_request`. Local `_GENERATE_REQUIRED` / `_GENERATE_OPTIONAL` type tables deleted (30 lines). `req.update(result.payload)` propagates the int→float safe cast into the caller's request dict per §3. Closes the bool-as-int subclass loophole at the IPC boundary (2026-04-23 §12 review finding 9). `security-auditor`: CLEAN with 2 MEDIUMs folded in.
  - **`comfyless/generate.py:_validate_iterate_value`** (step 4) — lora_stack branch delegates per-entry validation to `validate_lora_entry`. User-visible behavior tightening: missing `weight` in an `--iterate lora <file>.json` entry now rejects (was previously accepted with downstream `.get("weight", 1.0)` default). Authorized by §6 + Vision invariant 5.

  Cross-call-site invariants now active across all three consumers:
  - **N18** (cross-site accept/reject parity): 38 fixtures total — 26 canonical↔server full-request fixtures + 11 canonical↔iterate single-entry fixtures + 1 extra-keys preservation case.
  - **N19** (no forbidden `isinstance(int|float|bool|str)` in machine-boundary call sites): zero violations in `server.py:_validate_request` AND `generate.py:_validate_iterate_value`'s lora_stack branch. AST-based check, structurally tight.

  Total regression: 850 / 0 passing across all 8 test suites (118 validator + 92 iterate + 8 server-robustness + 135 params-schema + 186 manual-loop + 141 multistage + 129 cascade + 41 samplers).

  TECH_DEBT H-3 (`loras[i]["weight"]` not type-checked, 2026-04-23) flipped to `Resolved: 2026-05-16`. The 2026-04-23 §12 review's finding 9 (bool-as-int) also closed. One new TECH_DEBT entry added: server path-error audit log drops `prompt` only, not `negative_prompt` (pre-existing, surfaced by step-3 audit finding 8; deferred to next `_handle_connection`-touching commit).

  **Unblocks ADR-011 slice 1** — the MCP `generate` tool can now import `validate_machine_request` from `comfyless.params_validation` and inherit the canonical type contract from line one. Slice 1's request handler should follow step 3's precedent of propagating `result.payload` so MCP downstream consumers see the cast-applied payload.

- **2026-07-10 (amendment — `--iterate lora` reclassified as a human-replay surface)**: The step-4 decision to route the `--iterate lora <file>.json` per-entry check through the canonical `validate_lora_entry` was a **mis-scoping** and is reverted for that one call site. The reasoning is this ADR's own Context (§"Boundary discipline"): *machine-driven calls fail closed; human replay warns-and-keeps.* The `--iterate` file is a hand-authored, on-disk replay artifact — the same posture class as a `--params` sidecar — not a machine boundary. `--params` loras already default a missing `weight` to `1.0` (`generate._apply_loras`, `lora_spec.get("weight", 1.0)`); requiring an explicit `weight` in the iterate file made the human iterate path the single outlier in the ecosystem. Grant flagged this 2026-07-10; the fix decouples the human surface from the wire.

  **What changed (human iterate surface only):** `generate.py` gains `_normalize_iterate_lora_element`, a lenient normalizer used by `_plan_iterations` for the `lora` axis. It accepts, per element: a `"path"` or `"path:weight"` **string** (mirrors `_parse_lora_arg`, the CLI `--lora` grammar), a bare `{path, weight?}` **dict** (single-LoRA convenience), or a **list** of strings/dicts (an explicit reusable stack; `[]` = no LoRA). Missing `weight` defaults to `1.0` everywhere. `path` remains required and must be a non-empty string; a present `weight` must still be a real number, not `bool`/`str`/`None` (a garbled weight is an authoring error worth surfacing, not silently coercing). The normalized value stored in the plan is always the canonical `[{path: str, weight: float}, …]` stack, so nothing downstream of `_plan_iterations` changes.

  **What did NOT change (the actual machine boundary):** `validate_lora_entry` and `validate_machine_request` in `params_validation.py` are behaviorally unchanged — the IPC daemon socket and the MCP wire still fail closed on a missing `weight`. The only edit to that module is its docstring, corrected to drop the now-reclassified iterate consumer from its surface list (reviewer LOW; the validation logic is untouched). Agents on those surfaces already always emit `weight`; the strict posture there is correct and retained (Grant's 2026-07-10 decision: keep the wire strict). ADR-012's core decision — *one* canonical validator owns the machine boundary — stands; this amendment only removes a surface that never belonged in that set.

  **Invariant adjustments:**
  - **§"Deferred / Out of Scope" → cross-call-site parity** now commits to **two** machine sites (server + future MCP), not three. The iterate file is explicitly a human-replay surface and is NOT required to match the canonical validator.
  - **N18** (`test_machine_boundary_validator.py`): the 11 canonical↔iterate parity fixtures are removed; the 26 canonical↔server fixtures + extra-keys case remain. Iterate leniency is proven independently in `test_iterate.py` against `_normalize_iterate_lora_element`.
  - **N19**: the iterate-branch AST scan is removed (the normalizer legitimately does its own type checks now); the `server.py:_validate_request` isinstance-free scan is retained unchanged.
  - **TECH_DEBT H-3** stays `Resolved` — it was about the *machine boundary* type-checking `weight`, which it still does. This amendment is orthogonal.

  `code-reviewer` (Opus) run before commit; no `security-auditor` — the machine-boundary code (`params_validation.py`, `server.py`) is untouched by this amendment. AI-Disclosure: Claude (Opus 4.8, 1M context) authored; Grant reviewed.
