...
**Date:** 2026-06-27
**Status:** proposed (implementation gated by `code-reviewer` + `security-auditor`, both Opus — `_check_paths` lives in `comfyless/server.py`, a project-mandated security-review surface)
...
**Cross-kind overlap fails closed.** If the *same* `abs_path` is minted under two different kinds (operator passed overlapping `--lora-path`/`--transformer-path` trees), that is a kind ambiguity — first-in-wins would make the entry's kind depend on scan order. `CatalogBuildError` at spawn, naming the path and both kinds. Same-path/same-kind re-mints (one tree named twice, or nested roots of the same kind) remain harmless aliases.
