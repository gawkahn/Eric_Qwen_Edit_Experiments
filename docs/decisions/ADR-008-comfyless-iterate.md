...
- **2026-07-13** — added `--limit-per AXIS N`, a per-group truncation model, alongside (mutually exclusive with) `--limit`. Motivation: `--limit` truncates the *flattened* Cartesian, and since `itertools.product` varies the last axis fastest, `--iterate transformer_path t --iterate prompt p --limit 25` ran 25 prompts against only the *first* transformer. `--limit-per transformer_path 25` instead makes the named axis the OUTER group and runs the first N combinations of the OTHER axes for each group value — i.e. "25 prompts per transformer," leaving the prompt list intact. `effective_combos = len(AXIS) × min(N, ∏ other-axis lengths)`; the group axis varies slowest so the cycled axes are the same first-N chunk per group. Validated: AXIS must be an active `--iterate` axis, N ≥ 1; enforced mutually exclusive with `--limit` at the argparse layer; `--batch` and the `--max-iterations` fail-closed gate compose unchanged. `--limit`'s flat-ceiling semantics are untouched. `test_iterate.py` +35 tests (52→127 cumulative incl. earlier growth). Reviewed by code-reviewer (Fable).
