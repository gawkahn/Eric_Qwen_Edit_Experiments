...
## Output paths: `--output` vs `--savepath`

Two ways to direct output, each with collision-resistant numbering:

**`--output <dir-or-path>`** — literal path or directory. When pointed at a directory, cascade scans it for existing `cascade_NNNN.png` files at dispatch start and *continues* numbering from there. So a bash loop like:

```bash
for f in chunks/prompt*.json; do
    comfyless --model stablecascade c.json --iterate prompt "$f" \
        --output /out/sweep/  # same dir for all chunks
done
```

…produces `cascade_0001.png` … `cascade_0100.png` for the first chunk, then `cascade_0101.png` … `cascade_0200.png` for the second, with no clobbering. Sidecar JSONs ride alongside.

**`--savepath <template>`** — path template with token expansion. Cascade supports the same vocabulary as the standard comfyless dispatch:
- `%input%` — basename (sans extension) of the first `--iterate` axis's source file. With `--iterate prompt prompt1-100.json`, expands to `prompt1-100`.
- `%input_<axis>%` — basename for a specific iterate axis (e.g. `%input_seed%`).
- `%seed%` — the per-run seed.
- `%date:fmt%` — strftime expansion (e.g. `%date:%Y-%m-%d%`).
- `%model%` — always `stablecascade` for this dispatch.
- `%cfg%`, `%steps%` — map to `prior_cfg_scale` and `prior_steps`.

Example: `--savepath /out/sweep/%input%/cascade` produces `/out/sweep/prompt1-100/cascade_0001.png`, `/out/sweep/prompt101-200/cascade_0001.png`, etc. — per-chunk subdirectories with fresh numbering each. The auto-counter scans the resolved parent directory at every write, so collisions across reruns are also avoided.

Pick `--output` for "all images in one big dir, monotonic numbering"; pick `--savepath` for "template-driven directory layout."
